...
import com.vdn.lampbearer.action.actions.interaction.LighthouseDoorOpenAction;
...
public class LighthouseDoor extends Door {

    public LighthouseDoor(Position3D position3D) {
        super(position3D);
        ...
        setActions(List.of(LighthouseDoorOpenAction.getInstance()));
    }


    public LighthouseDoor(Position3D position3D, Arrangement arrangement) {
        super(position3D);

        BlockType blockType = arrangement instanceof VerticalArrangement ?
                BlockType.V_CLOSED_DOOR_LIGHTHOUSE : BlockType.H_CLOSED_DOOR_LIGHTHOUSE;
